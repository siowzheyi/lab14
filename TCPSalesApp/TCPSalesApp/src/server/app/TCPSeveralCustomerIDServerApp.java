...
public class TCPSeveralCustomerIDServerApp {
...
//				Customer customer = null;
                ...
                int numIds = dis.readInt();
                System.out.println("Request for customer ids: " + numIds);

                // Create an array to store the customers
                Customer[] customers = new Customer[numIds];

                // Process each customer name
                for (int i = 0; i < numIds; i++) {
                    int id = dis.readInt();
                    System.out.println("\nRequest for customer id: " + id);

                    // Get the customer from the data manager
                    customers[i] = manager.searchCustomerId(id);
                }
